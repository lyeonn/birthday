...
import { getPageTheme, THEME_MAP } from '@/lib/themes';
...
export default function StepReview() {
  ...
  const data = watch();
  const t = useMemo(
    () => getPageTheme(data.theme, data.color || '#FF6B9D'),
    [data.theme, data.color],
  );
  const themeDef = THEME_MAP[data.theme] ?? THEME_MAP.minimal;
  ...
  return (
    <div className="flex flex-col gap-3.5">
      <div
        ...
        style={{ background: t.pageBg, borderColor: t.cardBorder }}
      >
        <div
        ...
        <div className="relative mx-auto h-24 w-24">
          <div
            className="flex h-24 w-24 items-center justify-center rounded-full font-mono text-[9px]"
            style={{
              background: `repeating-linear-gradient(45deg, ${t.accent}22, ${t.accent}22 8px, ${t.accent}11 8px, ${t.accent}11 16px)`,
        ...
        <div
          className="mt-[18px] whitespace-pre-line font-display text-[17px] font-bold leading-[1.3] tracking-[-0.02em]"
      ...
      <div className="overflow-hidden rounded-cta border border-line bg-surface">
        ...
        <ReviewRow label="테마" value={`${themeDef.emoji} ${themeDef.name}`} swatch={t.accent} last />
      </div>
