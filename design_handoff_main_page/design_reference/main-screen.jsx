  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          onClick={() => onNav("share")}
          style={{ ...pillBtn(v), padding: "0 12px" }}
        >
          <Icons.Share />
        </button>
      </div>
      ...
      >
        HAPPY{" "}
        {getNthBirthday(friend.birthday)
          ? `${getNthBirthday(friend.birthday)}TH `
          : ""}
        BIRTHDAY · {friend.name.toUpperCase()}
      </div>
      ...
      >
        ...
        <div style={{ fontSize: 13, color: v.ink, lineHeight: 1.5 }}>
          {friend.hostName}님이 {friend.name}님을 위해 만든 페이지예요. 친구들이
          한 마디씩 남길 수 있어요!
        </div>
      </div>
    </div>
...
function MessagesPreview({ vibe, primary, friend, onNav }) {
  const v = VIBES[vibe];
  const messages = (
    typeof SAMPLE_MESSAGES !== "undefined" ? SAMPLE_MESSAGES : []
  ).slice(0, 3);
  return (
      ...
      >
        {messages.map((m) => (
          ...
          >
            ...
            >
              <div
                style={{ fontSize: 11, fontWeight: 700, color: v.ink + "cc" }}
              >
                {m.name}
              </div>
...
if (
  ...
  !document.getElementById("main-keyframes")
) {
